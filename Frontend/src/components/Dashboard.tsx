...
function Dashboard() {
    ...
    useEffect(() => {
        fetchMatches();
        // Removed aggressive auto-polling:
        // const interval = setInterval(fetchMatches, 300000); // Wait for manual refresh or initial load only
        // return () => clearInterval(interval);
    }, []);

    const handleRefreshMatch = async (matchId: string) => {
        try {
            const response = await fetch(`http://localhost:8000/api/match/${matchId}/refresh`);
            if (response.ok) {
                const updatedMatch = await response.json();

                // Update specific match in live matches
                setMatches(prev => ({
                    ...prev,
                    live: prev.live.map(m =>
                        m.id === matchId
                            ? { ...m, status: updatedMatch.status, score: updatedMatch.score }
                            : m
                    )
                }));
            }
                ...
                setMessages((prev) => {
                    ...
                    if (lastMsg.role === 'assistant') {
                        if (chunk.type === 'thought' || chunk.type === 'action' || chunk.type === 'observation') {
                            ...
                            // Pass approval attributes from the chunk mapping generated by main.py exception
                            if (chunk._requiresApproval) {
                                lastMsg._requiresApproval = true;
                                lastMsg._approvalAction = chunk._approvalAction;
                            }
    ...
    return (
        <div className="flex bg-zinc-50 h-screen w-screen overflow-hidden text-zinc-900 antialiased font-sans">
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
                ...
                {/* Left Panel: Chat (60%) */}
                <main className="flex-1 flex flex-col h-[60vh] lg:h-full bg-white lg:border-r border-zinc-200 relative min-w-0 lg:min-w-[400px]">
                        ...
                        <div className="flex flex-col min-h-full">
                            {messages.map((msg, index) => (
                                <ChatMessage
                                    ...
                                    message={msg}
                                    onApprove={() => {
                                        if (msg._requiresApproval) {
                                            // Handle Approval
                                            const lastUserMsg = messages.slice().reverse().find(m => m.role === 'user');
                                            const contextPrompt = lastUserMsg ? `\n\nOriginal Request: ${lastUserMsg.content}` : '';
                                            const approvalMsg = `I approve. Proceed with: ${msg._approvalAction}${contextPrompt}`;
                                            handleSendMessage(approvalMsg);
                                        }
                                    }}
                                    onReject={() => {
                                        if (msg._requiresApproval) {
                                            ...
                                            const lastUserMsg = messages.slice().reverse().find(m => m.role === 'user');
                                            const contextPrompt = lastUserMsg ? `\n\nOriginal Request: ${lastUserMsg.content}` : '';
                                            const rejectMsg = `No, I reject this action. Please stop and explain why this action was necessary.${contextPrompt}`;
                                            handleSendMessage(rejectMsg);
                                        }
                                    }}
                                />
                            ))}
                ...
                <aside className="w-full lg:w-[450px] bg-zinc-50/50 flex flex-col border-t lg:border-l lg:border-t-0 border-zinc-200 shrink-0 h-[40vh] lg:h-full">
                    <div className="p-4 border-b border-zinc-200 bg-white flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className="p-1.5 text-zinc-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors disabled:opacity-50"
                                title="Refresh Multiple Matches"
                            >
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        {/* Match List */}
                        <MatchList matches={matches} onRefreshMatch={handleRefreshMatch} />
                    </div>
                </aside>
